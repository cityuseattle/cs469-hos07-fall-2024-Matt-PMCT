...
class Graph():
    ...
    def find_min_distance_vertex(self, distance, shortest_path_set):
        # Initialize minimum distance for the next vertex
        min = INF_DISTANCE
    ...
    def dijkstra(self, source):
        ...
        for _ in range(self.V):
            # Pick the min distance vertex from the vertix set not yet processed
            ...
            # Update distance values of the picked vertex's adjacent vertices
            # only if the current distance is greater than new distance and
            # the vertex in not in the shortest path tree
            for v in range(self.V):
                if (self.graph[u][v] > 0 and shortest_path_set[v] == False and 
